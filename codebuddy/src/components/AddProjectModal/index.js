...
export default function ProjectModal({
  ...
  return (
    <>
      {type === "add" ? (
        <Button buttonType="filled-secondary" onClick={() => setOpen(true)}>
          Add +
        </Button>
      ) : (
        <div
          onClick={() => {
            type === "update"
              ...
              : Modal.confirm({
                  title: "Are you sure?",
                  content:
                    "You are about to remove your project, this cannot be undone.",
                  onOk: {},
                });
          }}
          className="flex items-center justify-center bg-light-400 dark:bg-dark-200 border border-light-200 dark:border-dark-100 h-8 w-8 rounded-lg cursor-pointer"
        >
          {type === "update" ? (
            <FiEdit className="text-dark-300 dark:text-white" />
          ) : (
            <FiTrash className="text-dark-300 dark:text-white" />
          )}
        </div>
      )}
      <Modal
